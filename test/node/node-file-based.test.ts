...
const provider = new FileBasedProvider("node", "test-data")
...
test("write and read software evaluation grids", async () => {
	...
	grid.updatedAt = gridRead.updatedAt;
...
test("write and read software evaluation", async () => {
	...
	await provider.pushSoftwareVersion(
		evaluation.software.type.uid,
		evaluation.software.uid,
		evaluation.softwareVersion
	)
	...
	evaluationRead.software.versions = evaluation.software.versions // versions are not returned by getEvaluation to save a useless request
